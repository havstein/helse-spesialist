...
internal class VurderÅpenGosysoppgaveTest {
    ...
    fun `Ber om åpne oppgaver i gosys`() {
        ...
        assertEquals(listOf("ÅpneOppgaver"), behov.keys.toList())
        assertEquals(skjæringstidspunkt.minusYears(1), behov["ÅpneOppgaver"]!!["ikkeEldreEnn"])
    }
    ...
    fun `Lagrer varsel ved åpne oppgaver, uavhengig om eventuell oppgave er tildelt eller ikke`() {
        ...
        clearMocks(dao)
        lagrerVarselVedÅpneOppgaver(true, commandContext())
    }
    ...
    private fun commandContext(behovsamler: MutableMap<String, Map<String, Any>>? = null) =
        CommandContext(UUID.randomUUID()).also { cc ->
            behovsamler?.let { cc.nyObserver(observer(behovsamler)) }
        }

    private fun observer(behovsamler: MutableMap<String, Map<String, Any>>) =
